...
def _detect_contradictions(results: list[RetrievalResult]) -> list[str]:
    text_by_doc = {result.chunk.doc_id: result.chunk.content.lower() for result in results}
    ...
    for left, right in CONTRADICTION_PAIRS:
        ...
        if left_docs and right_docs and set(left_docs) != set(right_docs):
            contradictions.append(
                f"Potential contradiction: '{left}' in {left_docs}; '{right}' in {right_docs}"
            )
...
def _detect_gaps(sub_queries: list[str], results: list[RetrievalResult]) -> list[str]:
    ...
    for query in sub_queries:
        keywords = [token.strip("?,.") for token in query.lower().split() if len(token) > 4]
