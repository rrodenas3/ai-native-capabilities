# Golden Principles — cap-03-agentic-commerce
...
test: grep -rn "claude-\|gpt-" cap-03-agentic-commerce/agents/ cap-03-agentic-commerce/tools/ --include="*.py" | grep -v "settings\|test\|#" | grep -v ".pyc"
severity: blocking
...
missing = []
for f in pathlib.Path('cap-03-agentic-commerce/agents').glob('*.py'):
    src = f.read_text()
...
test: grep -rn "\.invoke_tool\|\.call_tool\|requests\.get\|requests\.post" cap-03-agentic-commerce/agents/ --include="*.py" | grep -v "harness\|mcp\|test\|#"
severity: blocking
...
test: grep -rn "skip.*gate\|bypass.*gate\|if.*approved.*skip" cap-03-agentic-commerce/agents/ --include="*.py"
severity: blocking
