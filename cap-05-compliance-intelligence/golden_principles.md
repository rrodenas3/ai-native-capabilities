# Golden Principles — cap-05-compliance-intelligence
...
test: grep -rn "claude-\|gpt-" cap-05-compliance-intelligence/agents/ cap-05-compliance-intelligence/tools/ --include="*.py" | grep -v "settings\|test\|#" | grep -v ".pyc"
severity: blocking
...
missing = []
for f in pathlib.Path('cap-05-compliance-intelligence/agents').glob('*.py'):
    src = f.read_text()
...
test: grep -rn "\.invoke_tool\|\.call_tool\|requests\.get\|requests\.post" cap-05-compliance-intelligence/agents/ --include="*.py" | grep -v "harness\|mcp\|test\|#"
severity: blocking
...
test: grep -rn "skip.*gate\|bypass.*gate\|if.*approved.*skip" cap-05-compliance-intelligence/agents/ --include="*.py"
severity: blocking
