# Golden Principles — cap-02-agentic-engineering
...
test: grep -rn "claude-\|gpt-" cap-02-agentic-engineering/agents/ cap-02-agentic-engineering/tools/ --include="*.py" | grep -v "settings\|test\|#" | grep -v ".pyc"
severity: blocking
...
missing = []
for f in pathlib.Path('cap-02-agentic-engineering/agents').glob('*.py'):
    src = f.read_text()
...
test: grep -rn "\.invoke_tool\|\.call_tool\|requests\.get\|requests\.post" cap-02-agentic-engineering/agents/ --include="*.py" | grep -v "harness\|mcp\|test\|#"
severity: blocking
...
test: grep -rn "skip.*gate\|bypass.*gate\|if.*approved.*skip" cap-02-agentic-engineering/agents/ --include="*.py"
severity: blocking
