# Golden Principles — cap-04-autonomous-operations
...
test: grep -rn "claude-\|gpt-" cap-04-autonomous-operations/agents/ cap-04-autonomous-operations/tools/ --include="*.py" | grep -v "settings\|test\|#" | grep -v ".pyc"
severity: blocking
...
missing = []
for f in pathlib.Path('cap-04-autonomous-operations/agents').glob('*.py'):
    src = f.read_text()
...
test: grep -rn "\.invoke_tool\|\.call_tool\|requests\.get\|requests\.post" cap-04-autonomous-operations/agents/ --include="*.py" | grep -v "harness\|mcp\|test\|#"
severity: blocking
...
test: grep -rn "skip.*gate\|bypass.*gate\|if.*approved.*skip" cap-04-autonomous-operations/agents/ --include="*.py"
severity: blocking
